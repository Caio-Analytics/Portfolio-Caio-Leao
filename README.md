...
### [Bateia](https://github.com/Caio-Analytics/bateia) &middot; [dashboard ao vivo](https://caio-analytics.github.io/bateia/) &middot; [documentação dbt](https://caio-analytics.github.io/bateia/dbt/) &middot; [orquestração (Bateia Ops)](https://github.com/Caio-Analytics/bateia-ops)
Pipeline de dados e dashboard analítico sobre a mineração brasileira, sobre dados 100% reais e públicos da ANM (Agência Nacional de Mineração, via Relatório Anual de Lavra), cobrindo 2010 a 2025 em duas bases (Produção Bruta e Produção Beneficiada), cerca de 10.300 registros combinados. A camada Bronze fica em Python/Polars (decodifica CSV cp1252 do governo); Silver, Gold e o cruzamento entre as duas bases são inteiramente modelos dbt sobre DuckDB, com 58 testes de schema e documentação/linhagem geradas automaticamente. Dashboard executivo interativo num único arquivo HTML autocontido, sem backend. Integração contínua via GitHub Actions. O Bateia Ops pega o mesmo ETL e adiciona reprodutibilidade (Docker Compose) e orquestração (Dagster): cada camada vira um asset com lineage, retry automático e um check de qualidade.

### [colsemantics](https://github.com/Caio-Analytics/colsemantics) &middot; [PyPI](https://pypi.org/project/colsemantics/)
Biblioteca Python (`pip install colsemantics`) que infere o papel e o domínio de uma coluna a partir do nome, de abreviação corporativa e do conteúdo, sem dicionário fechado nem modelo de IA: cascata de 6 detectores independentes (padrão de conteúdo, dicionário, fuzzy Jaro-Winkler, gazetteer, assinatura estrutural, contexto da tabela) combinados por noisy-OR. Extraída do motor de inferência semântica do Recon como pacote standalone, tipada e testada.
